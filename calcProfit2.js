...
function parseSizes (currentStrategy, depth) {
  ...
  return [
    symbolsOrderBookInfoMap[buy].asks.slice(0, depth + 1 + firstStepDepth).reduce((res, item ) => res + parseFloat(item[1]), 0),
...
function calcProfit2(currentStrategy, orderBookDepth) {
